...
router.post('/orders/:id/reception-review', requireAnyRole(['receptionist']), idempotent('orders:reception-review'), orders.receptionReview);
router.post('/orders/:id/design-done', requireAnyRole(['designer']), idempotent('orders:design-done'), orders.designDone);
router.post('/orders/:id/production-done', requireAnyRole(['technician']), idempotent('orders:production-done'), orders.productionDone);
router.post('/orders/:id/qc-decision', requireAnyRole(['qc']), idempotent('orders:qc-decision'), orders.qcDecision);
router.post('/orders/:id/doctor-decision', requireAnyRole(['dentist']), idempotent('orders:doctor-decision'), orders.doctorDecision);
router.post('/orders/:id/confirm-completion', requireAnyRole(['receptionist']), idempotent('orders:confirm-completion'), orders.confirmCompletion);
router.post('/orders/:id/mark-delivered', requireAnyRole(['receptionist']), idempotent('orders:mark-delivered'), orders.markDelivered);
router.post('/orders/:id/mark-completed', requireAnyRole(['receptionist']), idempotent('orders:mark-completed'), orders.markCompleted);
...
// Assignment rosters — who reception/designers can hand an order to.
router.get('/staff', requireWorkflowRole(['receptionist', 'designer', 'technician', 'admin', 'lab']), orders.listStaff);
