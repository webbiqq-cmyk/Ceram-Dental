...
async function checkout(req, res) {
  ...
  const order = await orderModel.checkout(items, customer || {});
  if (!order) return bad(res, 'Could not place that order.');
  await notificationModel.notify('admin', { type: 'order-new', title: 'New shop order', body: order.id + ' — ' + (order.customer.name || 'Guest'), relatedId: order.id });
  ok(res, { order });
}
